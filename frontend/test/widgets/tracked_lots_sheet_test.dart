...
void main() {
  group('TrackedLotsBreakdown.parse — tolerant of the additive contract', () {
    ...
    test('missing untracked total falls back to the row sum; zero invested '
      ...
      (payload['untracked'] as List)
          .add({'symbol': 'CASH', 'value_usd': 5000.0});
  ...
  group('TrackedLotsSheet — rendering (en)', () {
    testWidgets(
        'title, caption, both symbols in order, plurals, first buy, '
      ...
      expect(_findText('Not included — no recorded purchase data'),
          findsOneWidget);

...
void main() {
  ...
  group('InstrumentDetailSheet', () {
    testWidgets(
      'renders header, price, stats, lot and account from a C-A payload',
      (tester) async {
  ...
  group('asset-class override editor (I2 / C3-C)', () {
    ...
    testWidgets(
      ...
      'the current class checked and NO Automatic row',
      (tester) async {
        useTallSurface(tester);
        await tester.pumpWidget(_host(sheet(_nvdaPayload())));
        await tester.pumpAndSettle();
        ...
        final bondsRowSize = tester.getSize(
          find
              .ancestor(of: find.text('Bonds'), matching: find.byType(InkWell))
              .first,
        );
    ...
    testWidgets('override payload: "manual" caption on the tile and an '
    ...
    testWidgets(
      ...
      'change and refetches on success',
      (tester) async {
        useTallSurface(tester);
        final payload = _nvdaPayload();
        final ranges = <String>[];
    ...
    testWidgets(
      'failed mutation reverts the chip and shows the error snackbar',
      (tester) async {
        useTallSurface(tester);
        var changed = false;
        await tester.pumpWidget(
  ...
  group('day-offset x-axis math (I3)', () {
    ...
    test(
      'dayOffsetSpots keeps real gaps: x is days since the first sample',
      () {

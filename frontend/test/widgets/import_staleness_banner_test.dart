...
void main() {
  testWidgets('renders nothing when no institution is stale', (tester) async {
    await tester.pumpWidget(_wrap(const ImportStalenessBanner(stale: [])));
  ...
  testWidgets('en: names the most-stale institution with its day count', (
    ...
    await tester.pumpWidget(_wrap(const ImportStalenessBanner(stale: _stale)));
    await tester.pumpAndSettle();
    final text = _allText(tester);
    expect(text, contains('Banamex data is 42 days old — import a statement'));
  ...
  testWidgets('en: singular day and no "more" line for a single institution', (
    ...
    await tester.pumpAndSettle();
    final text = _allText(tester);
    expect(text, contains('Nu México data is 1 day old — import a statement'));
  ...
  testWidgets('tapping the action fires onImport', (tester) async {
    ...
    await tester.pumpWidget(
      _wrap(
        ImportStalenessBanner(stale: _stale, onImport: () => tapped = true),
      ),
    );
    await tester.pumpAndSettle();
  ...
  testWidgets('dismiss × renders and reports the shown institutions', (
    ...
    expect(
      dismissed!.map((s) => s.name),
      ['Banamex', 'CetesDirecto'],
      reason:
          'the callback receives exactly the institutions listed, '
          'so the parent snoozes precisely that set',
    );
  });

  testWidgets('a muted institution is absent from the banner', (tester) async {

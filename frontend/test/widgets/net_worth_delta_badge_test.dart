...
NetWorthDeltaBadge _badge(NetWorthDelta delta) => NetWorthDeltaBadge(
      delta: delta,
      currencyFormat: NumberFormat.currency(locale: 'en_US', name: 'USD', symbol: r'$'),
      conversionFactor: 1.0,
    );

void main() {
  group('NetWorthDeltaBadge — honest anchor label', () {
    test('badge label reuses pfDeltaVsAgo — same copy as the chart chip',
        () async {
    ...
    testWidgets('7d fallback anchor is labeled "vs 7d ago" (en)',
        (tester) async {
      await tester.pumpWidget(_host(_badge(
    ...
    testWidgets('30d anchor is labeled "vs 30d ago" (en)', (tester) async {
      await tester.pumpWidget(_host(_badge(
        const NetWorthDelta(amount: -120, percentage: -1.5, windowLabel: '30d'),
      )));
      await tester.pumpAndSettle();
    ...
    testWidgets('unreliable baseline (null %) shows the dollar delta alone',
        (tester) async {
      await tester.pumpWidget(_host(_badge(
        const NetWorthDelta(amount: 500, percentage: null, windowLabel: '30d'),
      )));
      await tester.pumpAndSettle();

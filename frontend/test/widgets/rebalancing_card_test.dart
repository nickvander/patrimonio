...
List<AllocationData> _googHeavyAllocation() => [
  ...
  AllocationData('Cash', 'USD', 150000, Colors.blue, assetClassKey: 'cash'),
  AllocationData(
    'Crypto',
    'BTC',
    30000,
...
void main() {
  ...
  group('RebalancingCard editor', () {
    testWidgets('sum-to-100 validation: Save gated on the meter, Distribute '
      ...
      expect(
        find.text(r'Move $5,000.00 from Cash to Stocks & funds'),
        findsOneWidget,
      );
  ...
  group('RebalancingCard drift rendering', () {
    testWidgets(
      ...
      'big delta chip, dollar guidance, no overflow',
      (tester) async {
        ...
        expect(
          find.text(r'Move $150,000 from Stocks & funds to Bonds'),
          findsOneWidget,
        );
        expect(
          find.text(r'Move $45,000 from Stocks & funds to Crypto'),
          findsOneWidget,
        );
    ...
    testWidgets('holdings-sourced unclassified (legacy backend, no asset_class) '

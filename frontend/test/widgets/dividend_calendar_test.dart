...
String _entryDateLabel(int offset) {
  final now = DateTime.now();
  return DateFormat.yMMMd().format(DateTime(now.year, now.month + offset, 14));
}
  ...
  for (var i = 0; i < 12; i++)
    {
      ...
      'entries': [_entry('O', i, 10.0), if (i % 3 == 0) _entry('KO', i, 3.10)],
    },
];
...
void _expectNotEllipsized(WidgetTester tester, Finder finder) {
  ...
  for (final element in elements) {
    ...
    expect(
      ...
      reason:
          '"${text.data}" needs ${painter.width}px but only got '
...
void main() {
  ...
  group('DividendCalendar (contract C4-B, bar-list redesign)', () {
    ...
    testWidgets('all-zero calendar → 12 em-dash rows, no bars, caption kept', (
      tester,
    ) async {
      _useSurface(tester, const Size(1440, 900));
      ...
      for (var i = 0; i < 12; i++) {
        final bar = tester.widget<FractionallySizedBox>(find.byKey(_barKey(i)));
        expect(
          bar.widthFactor,
          0.0,
    ...
    testWidgets(
      ...
      'the first (single-expansion invariant)',
      (tester) async {
        _useSurface(tester, const Size(1440, 900));
    ...
    testWidgets('REGRESSION (stagger bug): at 390px every row spans the full '
    ...
    testWidgets('bars share one global scale; zero months are inert', (
      ...
      double factor(int i) => tester
          .widget<FractionallySizedBox>(find.byKey(_barKey(i)))
          .widthFactor!;
      ...
      final dryInkWell = tester.widget<InkWell>(
        find.descendant(
          of: find.byKey(_rowKey(3)),
          matching: find.byType(InkWell),
        ),
      );
      expect(dryInkWell.onTap, isNull);
      // Merging nodes surface their content via getSemanticsData().
      final dryData = tester
          .getSemantics(find.byKey(_rowKey(3)))
          .getSemanticsData();
    ...
    testWidgets('converts every figure through conversionFactor + format', (
      ...
      await tester.pumpWidget(
        _host(
          calendar(_quarterlyCalendar(), conversionFactor: 20.0, format: mxn),
        ),
      );
    ...
    testWidgets(
      ...
      (tester) async {
        ...
        final row = tester
            .getSemantics(find.byKey(_rowKey(0)))
            .getSemanticsData();
        expect(
          row.label,
        ...
        final dry = tester
            .getSemantics(find.byKey(_rowKey(1)))
            .getSemanticsData();
        ...
        final entryLabel = tester
            .getSemantics(find.text('ABBV'))
            .getSemanticsData()
            .label;
    ...
    testWidgets('two-column split is driven by the inner CONSTRAINT (>= 720), '
    ...
    testWidgets('FIX (390px ellipsis): expanded panel shows the full date '
    ...
    testWidgets(
      ...
      (tester) async {
        ...
        final handle = tester.ensureSemantics();
        await tester.pumpWidget(
          _host(calendar(_crowdedCalendar()), locale: const Locale('es')),
        );
  ...
  group('DividendIncomeCard calendar hook (C4-B back-compat)', () {
    ...
    testWidgets('field absent (round-3 backend) → no toggle, card unchanged', (
      tester,
    ) async {
      _useSurface(tester, const Size(1440, 900));
    ...
    testWidgets(
      ...
      (tester) async {
        _useSurface(tester, const Size(1440, 900));
        await tester.pumpWidget(
          _host(card(_cardPayload(calendar: _mixedCalendar()))),
        );
        await tester.pump();

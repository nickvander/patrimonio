...
void main() {
  group('holdingMatchesCategoryFilter', () {
    ...
    test(
      'asset: ignores a bonds-typed ACCOUNT (the match-everything repro)',
      () {
    ...
    test('account_type: matches only account_type', () {
      ...
      final iraNamedInstitution = _h(
        accountType: 'brokerage',
        institution: 'ira',
      );
      expect(holdingMatchesCategoryFilter(inIra, 'account_type:ira'), isTrue);
      expect(
        holdingMatchesCategoryFilter(iraNamedInstitution, 'account_type:ira'),
        isFalse,
      );
    });
    ...
    test('institution: matches only institution_name, case-insensitively', () {
      ...
      expect(
        holdingMatchesCategoryFilter(vanguard, 'account_type:vanguard'),
        isFalse,
      );
    });
    ...
    test('institution values with spaces stay scoped to their dimension', () {
      ...
      expect(
        holdingMatchesCategoryFilter(schwab, 'institution:Charles Schwab'),
        isTrue,
      );
    });
    ...
    test('bare value falls back to legacy any-field OR', () {
      ...
      expect(
        holdingMatchesCategoryFilter(_h(holdingType: 'etf'), 'bonds'),
        isFalse,
      );
    });

    test(
      'unknown prefix falls back to legacy exact match on the whole value',
      () {
    ...
    test('colon after the first space is not a dimension prefix', () {
      ...
      expect(
        holdingMatchesCategoryFilter(odd, 'Banco Azteca: Premium'),
        isTrue,
      );
    });
  });
  ...
  group('categoryFilterLabel', () {
    test('strips known prefixes and title-cases', () {
      ...
      expect(
        categoryFilterLabel('institution:charles schwab'),
        'Charles Schwab',
      );
    });

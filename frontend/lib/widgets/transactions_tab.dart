...
class TransactionsTabState extends State<TransactionsTab>
    implements TransactionDetailHost {
  String _searchQuery = '';
  ...
  /// [TransactionDetailPanel] inside a slide-in side panel (wide) or
  ...
  // The detail panel (transaction_detail_panel.dart, decoupled 2026-08-03)
  // consumes the tab exclusively through this surface. Thin delegates
  // only: every helper stays single-source above, and the in-file call
  // sites keep using the private originals. `context`/`mounted` need no
  // delegate — State's own satisfy the interface (the panel's post-close
  // SnackBars deliberately ride the TAB's lifetime, not the panel's).
  ...
  @override
  List<dynamic> get transactions => widget.transactions;
  ...
  get onSplitTransaction => widget.onSplitTransaction;

  @override
  Future<void> Function(String parentId)? get onUnsplitTransaction =>
      widget.onUnsplitTransaction;

  @override
  Future<void> Function(String id)? get onDelete => widget.onDelete;

  @override
  void Function(dynamic tx)? get onCreateLoanFromTx =>
      widget.onCreateLoanFromTx;

  @override
  void Function(dynamic tx)? get onMakeRecurring => widget.onMakeRecurring;

  @override
  List<String> distinctCategories() => _distinctCategories();

  @override
  String accountLabel(dynamic tx) => _accountLabel(tx);

  @override
  Widget sectionLabel(String label) => _sectionLabel(label);

  @override
  Widget metaChip(IconData icon, String label) => _metaChip(icon, label);
  ...
  @override
  IconData channelIcon(String channel) => _channelIcon(channel);

  @override
  String sentenceCase(String s) => _sentence(s);

  @override
  Widget similarRow(dynamic tx) => _similarRow(tx);

  @override
  List<Widget> fxTransferBlock(String txId) => _fxTransferBlock(txId);

  @override
  void openEditManualDialog(dynamic tx) => _openEditManualDialog(tx);

...
pub fn parse(data: &[u8]) -> Result<Vec<ParsedTransaction>> {
    ...
    info!("Extracted {} characters from Banamex PDF", full_text.len());
...
pub fn parse_text(text: &str) -> Result<Vec<ParsedTransaction>> {
    ...
    // Pattern for Banamex (approximated):
    // "15 MAR COMPRA OXXO $ 50.00"
    // Similar to Nu Mexico but maybe different month names or separators.
    let re = Regex::new(r"(?P<day>\d{1,2})\s+(?P<month>[A-Z]{3})\s+(?P<desc>.+?)\s+[\$]?\s*(?P<amount>-?\d[\d,.]*\.\d{2})")?;
    ...
    if transactions.is_empty() {
        ...
        info!("No transactions found in Banamex PDF. Snippet: {}", snippet);
    }

...
pub async fn sync_all_institutions(db: &PgPool, config: &AppConfig) -> Result<()> {
    ...
    let rows = sqlx::query(
        "SELECT id, name, integration_type, plaid_access_token_enc FROM institutions"
    )
    .fetch_all(db)
    .await?;
                        ...
                        if let Some(added) = tx_val["added"].as_array() {
                            for tx in added {
                                ...
                                if let Some(acc_row) = internal_acc {
                                    ...
                                    let _ = sqlx::query(
                                        "INSERT INTO transactions (account_id, external_id, date, description, amount) VALUES ($1, $2, $3, $4, $5)"
                                    )
                        ...
                        if let Some(holdings) = hold_val["holdings"].as_array() {
                            for h in holdings {
                                ...
                                if let Some(acc_row) = internal_acc {
                                    let acc_id: uuid::Uuid = acc_row.get("id");
                                    let _ = sqlx::query(
                                        "INSERT INTO holdings (account_id, symbol, name, quantity, price, value, cost_basis) VALUES ($1, 'SEC', 'Investment', $2, $3, $4, $5)"
                                    )

...
## DEC-027: User Rules Apply at Write Time, Gated by a Single-Use Preview Token
**Date:** 2026-08-03
**Status:** Accepted (design signed off; implementation in progress)
**Context:** The feature-research sweep's top-evidence brief asked for user-defined categorization/rename rules. Patrimonio already has an *invisible* version — `imports.rs` learn-from-edits stamps `user_category` from past manual edits on the statement-import path only — so the design had to justify its delta and, above all, guarantee that a rule never silently rewrites spending history (which would move tax and cash-flow totals under the owner).
**Decision:** Rules apply at **write time** (statement-import confirm + the previously-uncovered Plaid sync path), never at read time — every effective-value reader (`EFFECTIVE_CATEGORY_SQL`, spending, tax, exports, frontend ladder) keeps consuming the stored `user_category`/`user_description` columns, so there is nothing to re-implement per reader. Retroactive application is the **only** path that touches existing rows and requires a Redis-stored, fingerprint-matched, single-use preview token: the confirmed diff *is* the write, a rule edited after preview 409s, and rows manually edited between preview and apply are skipped by a SQL predicate rather than clobbered. New provenance columns (`user_category_source` / `user_description_source` ∈ manual|rule|learned, plus rule ids) make "a human edit always wins" an enforceable predicate instead of a convention; legacy values are conservatively treated as manual. The learned-map source query excludes rule/learned rows so rule output can't feed back and resurrect a deleted rule.
**Alternatives:** Read-time rule evaluation (rejected — changes historical aggregates the instant a rule is saved, the exact silent rewrite the brief forbids, and would need re-implementing in every SQL reader); new `rule_*` columns beside the user ones (rejected — forces edits to every read site, the hand-synced-copies drift class); regex matchers in v1 (rejected — a backtracking pattern would wedge the per-row sync path, and contains/starts_with/merchant_key already cover the es-MX descriptor cases; revisit if a real descriptor defeats them).

---

## DEC-028: Rules May Recategorize FX-Transfer Legs (With a Diff Warning); Deleting a Rule Keeps Its Applied Values
**Date:** 2026-08-03
**Status:** Accepted
**Context:** Two owner-facing product calls from the rules-engine design. (1) Cross-currency transfer legs are excluded from cash-flow spending via category anti-joins, so letting a rule recategorize one silently re-enters it into spending totals. (2) On rule deletion, the values a rule already wrote could persist or be reverted.
**Decision:** (1) **Allow + warn**: rules may match FX-transfer legs, and the dry-run response carries an `fx_transfer_legs` count that the diff renders as an explicit warning banner — the owner decides per rule, with the consequence visible before confirming. Blocking stays a one-line predicate if the warning proves insufficient. (2) **Applied values persist** after a rule is deleted (Monarch-style); they keep `*_source='rule'` with a NULL rule id, so they remain machine-owned and freely overwritable by a future rule, while a human edit is still untouchable. A dry-runnable "revert this rule's applications" stays out of the MVP.
**Rationale:** The dry-run already prevents the scenario revert exists for — no rule reaches history without a previewed, confirmed diff — and the natural correction for a wrong rule is a corrected rule applied over the same rows, which the provenance model permits. Revert is a well-defined follow-up if a real case ever bites, so this is "not now", not "never".
**Trade-off:** A recategorized transfer leg will change cash-flow totals; the warning is the only guard. Deleted-rule residue is invisible in the UI until some rule or manual edit overwrites it.
